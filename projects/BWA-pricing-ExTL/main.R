...
records <- read.csv("../BWA-measurement/results/all.tsv", sep="\t")
...
price.ExTL <- function(itype, concurrency, time) {
  ...
  computing.instances <- EC2.price.for("m4.large") * time * concurrency
...
sapply(seq(0,3), function(i) {
  c <- 2^i
  results <<- rbind(results, data.frame(instance_type = "ETL", concurrency = c, avg = price.ETL(c, ETL.avg), min = price.ETL(c, ETL.min), max = price.ETL(c, ETL.max), row.names = NULL))
})
